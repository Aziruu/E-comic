        ...
        <div class="col-xxl-3 col-xl-4 col-lg-6 col-md-6 mb-4">
            <div class="card stretch stretch-full">
                <div class="position-relative" style="height: 300px; overflow: hidden;">
                    @if($book->primary_cover)
                        <img src="{{ asset('storage/' . $book->primary_cover->image_path) }}" 
                             ...
                             alt="{{ $book->title_primary }}">
                    @else
                        <div class="d-flex align-items-center justify-content-center bg-gray-200 h-100 w-100 text-muted">
                            <i class="feather-image fs-1"></i> No Cover
                        </div>
                    @endif
                    ...
                    <span class="badge bg-warning text-dark position-absolute top-0 start-0 m-3">
                        <i class="feather-star me-1"></i> {{ $book->rating ?? 'N/A' }}
                    </span>
                </div>

                <div class="card-body">
                    <h5 class="text-truncate-1-line mb-1">
                        <a href="{{ route('books.show', $book->slug) }}" class="text-dark">{{ $book->title_primary }}</a>
                    </h5>
                    <p class="text-muted fs-12 text-truncate-1-line mb-3">{{ $book->title_secondary ?? '-' }}</p>

                    <div class="d-flex justify-content-between align-items-center">
                        <div class="fs-12">
                            <i class="feather-book-open me-1"></i> Ch. {{ $book->last_read_chapter ?? 0 }} / {{ $book->total_chapters }}
                        </div>
                ...
                <div class="card-footer d-flex justify-content-between">
                    ...
                    <form action="{{ route('books.destroy', $book->id) }}" method="POST" onsubmit="return confirm('Yakin mau hapus manga ini?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-sm btn-light-danger"><i class="feather-trash-2"></i></button>
                    </form>
                </div>
            </div>
        </div>
